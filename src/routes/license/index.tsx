	...
	return (
		<main className="mt-32 mb-32 flex h-fit min-h-svh flex-col items-start gap-16 px-6 sm:mt-40 sm:gap-20 sm:px-10 md:mt-48 md:gap-24 md:px-14 lg:mt-56 lg:gap-28 lg:px-20 xl:px-24">
				...
				<ul className="flex flex-col gap-4">
					{logoLicense.map((license) => (
						<li key={license.name} className="flex flex-col gap-0">
							...
							<Link
								to={license.licenseLink}
								className="relative w-fit leading-snug duration-200 ease-out after:absolute after:bottom-0 after:left-0 after:z-[-1] after:h-0.5 after:w-full after:origin-right after:scale-x-0 after:bg-violet-500 after:transition-transform after:duration-300 after:ease-custom hover:after:origin-left hover:after:scale-x-100 focus-visible:after:origin-left focus-visible:after:scale-x-100 after:motion-reduce:duration-0"
								target="_blank"
								rel="noopener noreferrer"
							>
								{license.license}
			...
			<p className="text-lg tracking-tight">
				...
				>
					info@raflimalik.com{""}
					<span className="absolute bottom-0 left-0 z-[-1] h-0 w-full bg-theme-purple transition-all duration-200 ease-custom group-hover:h-full group-focus-visible:h-full dark:bg-violet-700"></span>
				</a>
			</p>
		</main>
	);
